...
struct Job bgJobs[1024];
int bgJobid = 0;

int parse(struct Job* jobs) {
	...
	while(!(thisArg == NULL)) {
		...
		if (strcmp(thisArg, "&") == 0) {
			...
			strcpy(thisArg, jobs[currJob].input); //open file in read only
			if (strlen(jobs[currJob].input) == 0) {
				perror ("Error opening input file\n");
			}
		} else if (strcmp(thisArg, ">") == 0) {
			thisArg = strtok(NULL, " =\n");	//grab next argument (should be the file name)
			strcpy(jobs[currJob].output, thisArg); //open file in write only
...
int execute(struct Job* jobs, int numJobs) {
		
	int exitbit = 0;	
	int pipefd[numJobs-1][2];
	
	int i; for(i = 0; i < numJobs-1; i++) {
		if (pipe(pipefd[i]) < 0) {
	...
	for(i = 0; i < numJobs; i++) {		
		...
		// Return PATH or HOME
			...
			if (pid == 0) {
				...
				if ((!strlen(jobs[i].input)) == 0 || (!strlen(jobs[i].output) == 0)) { //remember by default all id's > 0 so 0 means empty
				printf("using < or >\n");
					if (!strlen(jobs[i].input) == 0) {	// Redirect standard in (due to '<')
						printf("input not empty\n");
						FILE *f = fopen(jobs[i].input, "r");
						dup2(fileno(f), STDIN_FILENO);
						fclose(f);
						//set up pipe to read from job[inPipeId]
					}
					if (!strlen(jobs[i].output) == 0){	// Redirect standard out (due to '>')
						printf("output not empty\n");
						FILE *f = fopen(jobs[i].output, "w+");
	...
	while(exitbit == 0) {
		...
		if(exitbit == 0) {
			for(i = 0; i < numJobs; i++) {
				...
				jobs[i].input = (char*) malloc(10000);
				jobs[i].output = (char*) malloc(10000);
			}
		}
	}
	return 0;
